...
public class VoipTask extends AbstractQoSTask {
    ...
    // jitter + packet loss
    public final static String RESULT_PACKET_LOSS = "_packet_loss";
    public final static String RESULT_JITTER = "_jitter";
    ...
    public QoSTestResult call() throws Exception {
        ...
        try {
			...
			*/
            ...
            final String voipOutgoingPrefix = RESULT_VOIP_PREFIX + RESULT_OUTGOING_PREFIX;
            ...
            final ControlConnectionResponseCallback incomingResultRequestCallback = new ControlConnectionResponseCallback() {
                ...
                public void onResponse(final String response, final String request) {
                    if (response != null && response.startsWith("VOIPRESULT")) {
                        ...
                        if (m.find()) {
                            result.getResultMap().put(voipOutgoingPrefix + RESULT_MAX_JITTER, Long.parseLong(m.group(1)));
                            result.getResultMap().put(voipOutgoingPrefix + RESULT_MEAN_JITTER, Long.parseLong(m.group(2)));
                            result.getResultMap().put(voipOutgoingPrefix + RESULT_MAX_DELTA, Long.parseLong(m.group(3)));
                            result.getResultMap().put(voipOutgoingPrefix + RESULT_SKEW, Long.parseLong(m.group(4)));
                            result.getResultMap().put(voipOutgoingPrefix + RESULT_NUM_PACKETS, Long.parseLong(m.group(5)));
                            result.getResultMap().put(voipOutgoingPrefix + RESULT_SEQUENCE_ERRORS, Long.parseLong(m.group(6)));
                            result.getResultMap().put(voipOutgoingPrefix + RESULT_SHORT_SEQUENTIAL, Long.parseLong(m.group(7)));
                            result.getResultMap().put(voipOutgoingPrefix + RESULT_LONG_SEQUENTIAL, Long.parseLong(m.group(8)));
                        }
            ...
            final String voipIncomingPrefix = RESULT_VOIP_PREFIX + RESULT_INCOMING_PREFIX;

            if (rtpResults != null) {
                result.getResultMap().put(voipIncomingPrefix + RESULT_MAX_JITTER, rtpResults.getMaxJitter());
                result.getResultMap().put(voipIncomingPrefix + RESULT_MEAN_JITTER, rtpResults.getMeanJitter());
                result.getResultMap().put(voipIncomingPrefix + RESULT_MAX_DELTA, rtpResults.getMaxDelta());
                result.getResultMap().put(voipIncomingPrefix + RESULT_SKEW, rtpResults.getSkew());
                result.getResultMap().put(voipIncomingPrefix + RESULT_NUM_PACKETS, rtpResults.getReceivedPackets());
                result.getResultMap().put(voipIncomingPrefix + RESULT_SEQUENCE_ERRORS, rtpResults.getOutOfOrder());
                result.getResultMap().put(voipIncomingPrefix + RESULT_SHORT_SEQUENTIAL, rtpResults.getMinSequential());
                result.getResultMap().put(voipIncomingPrefix + RESULT_LONG_SEQUENTIAL, rtpResults.getMaxSequencial());
            } else {
                result.getResultMap().put(voipIncomingPrefix + RESULT_MAX_JITTER, null);
                result.getResultMap().put(voipIncomingPrefix + RESULT_MEAN_JITTER, null);
                result.getResultMap().put(voipIncomingPrefix + RESULT_MAX_DELTA, null);
                result.getResultMap().put(voipIncomingPrefix + RESULT_SKEW, null);
                result.getResultMap().put(voipIncomingPrefix + RESULT_NUM_PACKETS, 0);
                result.getResultMap().put(voipIncomingPrefix + RESULT_SEQUENCE_ERRORS, null);
                result.getResultMap().put(voipIncomingPrefix + RESULT_SHORT_SEQUENTIAL, null);
                result.getResultMap().put(voipIncomingPrefix + RESULT_LONG_SEQUENTIAL, null);
            }

            // jitter
            String jitter = getMeanJitter(
                    (Long) result.getResultMap().get(voipOutgoingPrefix + RESULT_MEAN_JITTER),
                    (Long) result.getResultMap().get(voipIncomingPrefix + RESULT_MEAN_JITTER));
            result.getResultMap().put(RESULT_VOIP_PREFIX + RESULT_JITTER, jitter);

            // packet loss
            String packetLoss = getMeanPacketLossInPercent(
                    (Integer) result.getResultMap().get(voipIncomingPrefix + RESULT_NUM_PACKETS),
                    (Long) result.getResultMap().get(voipOutgoingPrefix + RESULT_NUM_PACKETS),
                    callDuration, delay);
            result.getResultMap().put(RESULT_VOIP_PREFIX + RESULT_PACKET_LOSS, packetLoss);
    ...
    public String getMeanJitter(Long resultOutMeanJitter, Long resultInMeanJitter) {
        ...
        try {
            if (resultInMeanJitter != null && resultOutMeanJitter != null && resultInMeanJitter >= 0l && resultOutMeanJitter >= 0l) {
                ...
                meanJitterFormattedString = df.format(meanJitter / 1000000);
            }
        } catch (Exception ex) {
            //ex.printStackTrace();
    ...
    public String getMeanPacketLossInPercent(Integer resultInNumPackets, Long resultOutNumPackets, Long objectiveCallDuration, Long objectiveDelay) {
        ...
        try {
            ...
            int packetLossDown = (int) ((float) (total - resultInNumPackets) / (float) total);
            int packetLossUp = (int) ((float) (total - resultOutNumPackets) / (float) total);
            if ((packetLossDown >= 0) && (packetLossUp >= 0)) {
                double meanPacketLoss = (packetLossDown + packetLossUp) / 2;
                DecimalFormat df = new DecimalFormat("0.0");
                packetLossStr = df.format(meanPacketLoss);
            }
        } catch (Exception ex) {
            //ex.printStackTrace();
            System.err.println(ex.getMessage());
