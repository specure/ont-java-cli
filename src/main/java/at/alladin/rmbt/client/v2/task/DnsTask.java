...
public class DnsTask extends AbstractQoSTask {
    ...
    public final static String DEFAULT_RESOLVER = "8.8.8.8";
    ...
    public static List<JSONObject> lookupDns(String domainName, String record, String resolver, int timeout, QoSTestResult testResult) {
        ...
        try {
            if (resolver == null || resolver.isEmpty()) {
                String localResolver = getResolver();
                resolver = localResolver != null ? localResolver : DEFAULT_RESOLVER;
            }
    ...
    private static String getResolver() {
        ResolverConfig resolverConfig = ResolverConfig.getCurrentConfig();
        String resolver = DEFAULT_RESOLVER;
        if (resolverConfig != null) {
            resolverConfig.refresh(); // refresh dns server
            for (InetSocketAddress inetSocketAddress : resolverConfig.servers()) {
                try {
                    if (inetSocketAddress.getAddress().isReachable(100)) {
                        resolver = inetSocketAddress.getHostName();
                        break;
                    }
                } catch (IOException e) {
                    e.printStackTrace();
    ...
    public QoSTestResult call() throws Exception {
        ...
        try {
            ...
            testResult.getResultMap().put(RESULT_RESOLVER, resolver != null && resolver.compareTo("localhost") != 0 ? resolver : "Standard");
            testResult.getResultMap().put(RESULT_RESOLVER, "Standard");
