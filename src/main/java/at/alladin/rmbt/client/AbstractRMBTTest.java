...
public abstract class AbstractRMBTTest {
    ...
    protected Socket getSocket(final String host, final int port, final boolean isSecure, final int timeOut) throws UnknownHostException, IOException {
        ...
        if (socket != null) {
            log("Connecting to " + sockAddr + " with timout: " + timeOut + "ms " + socket + " [SSL: " + isSecure + "] ...");
            try {
                socket.connect(sockAddr, timeOut);
                //log("Connecting to " + sockAddr + " with timout: " + timeOut + "ms " + socket + " [SSL: " + isSecure + "] ... DONE");
            } catch (Exception e) {
                //log("An error occurred during connecting to " + sockAddr + "! Exception: " + e.getMessage());
            }
        }
