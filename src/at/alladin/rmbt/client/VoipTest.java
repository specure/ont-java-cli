...
import at.alladin.rmbt.client.v2.task.*;
...
public class VoipTest extends QualityOfServiceTest implements Callable<QoSResultCollector> {
    ...
    public VoipTest(RMBTClient client, TestSettings nnTestSettings, boolean onlyVoipTest) {
     ...
     * @param client
     */
    public VoipTest(RMBTClient client, TestSettings nnTestSettings) {
                ...
                if (controlConnection.getConcurrencyGroupSet().size() > 0) {
                    if (openAll) {
                        ...
                        if (controlConnection.getConcurrencyGroupSet().last() == concurrencyGroup) {
                            if (controlConnection.isRunning.get()) {
